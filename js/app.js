(function () {
  ...
  function absolutizeAssets(scope) {
    if (!scope) return;
    ...
    const normalize = (value) => {
      ...
      if (!next.startsWith('assets/')) return null;
      return new URL(next, assetRoot).href;
    };
    const setAbs = (el, attr) => {
      const v = el.getAttribute(attr);
      const abs = normalize(v);
      if (abs) el.setAttribute(attr, abs);
    };
    scope.querySelectorAll('[href]').forEach((el) => setAbs(el, 'href'));
    scope.querySelectorAll('[src]').forEach((el) => setAbs(el, 'src'));
  }
  Library.absolutizeAssets = absolutizeAssets;
  ...
  function bindTranslateControls(root) {
    ...
    function isTranslated() {
      ...
      return (
        h.includes("translate.google") || h.includes("googleusercontent.com")
      );
    }
    ...
    function originalFromTranslateUrl() {
      ...
      if (sp.has("u")) {
        try {
          return decodeURIComponent(sp.get("u"));
        } catch {
          return sp.get("u");
        }
      }
...
(function () {
  ...
  Library.injectMenu = async function (slotSelector) {
    try {
      ...
      if (!slot) return;
      const res = await fetch(new URL('../views/partials/menu.html', import.meta.url), {
        cache: 'no-store',
      });
      const html = await res.text();
      slot.innerHTML = html;
      if (Library.absolutizeAssets) Library.absolutizeAssets(slot);
      const viewsBase = new URL('../views/', import.meta.url);
      slot.querySelectorAll('[data-view]').forEach((el) => {
        const view = el.getAttribute('data-view');
        if (!view) return;
        el.setAttribute('href', new URL(`${view}.html`, viewsBase).href);
      });
      const scope = slot;
      // wire and bind controls within the slot
      if (Library.wireMenu) Library.wireMenu(scope);
      if (Library.bindMenuControls) Library.bindMenuControls(scope);
    } catch (e) {
...
(async function () {
  ...
  try {
    const res = await fetch(new URL('../views/partials/favicon.html', import.meta.url));
    if (res.ok) {
      const container = document.createElement('div');
      container.innerHTML = await res.text();
      if (Library.absolutizeAssets) Library.absolutizeAssets(container);
      document.getElementById('favicon-slot').outerHTML = container.innerHTML;
    }
  } catch (e) {
