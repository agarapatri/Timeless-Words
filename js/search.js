...
(function () {
  ...
  function ensurePager() {
    ...
    if (!pager && list) {
      ...
      pager.innerHTML = `
        ...
        <select id="pageSize" class="btn ghost" aria-label="Results per page">
  ...
  function changePage(delta) {
    const totalPages = Math.max(
      1,
      Math.ceil(STATE.results.length / STATE.perPage)
    );
  ...
  async function loadVerses() {
    ...
    if (wfw.rows)
      for (const row of wfw.rows) ALL.wfwByVerse.set(row[0], row[1] || "");
  }
  ...
  async function ensureSemanticReady() {
    ...
    if (!window.__twEncoder || !window.__twSemdb) {
      const [{ TransformerEncoder }, { SemanticDB }] = await Promise.all([
        import("./transformer_encoder.js?v=cadeb923"),
        import("./vec_db.js?v=cadeb923"),
      ]);
  ...
  function pickSnippets(book, verse, f, q) {
    ...
    const fields = {
      deva: () => ({
        key: "deva",
        label: "Devanāgarī",
        text: verse.devanagari || verse.original_sanskrit || "",
      }),
      iast: () => ({
        key: "iast",
        label: "IAST",
        text: verse.iast || verse.iast_transliteration || "",
      }),
      trans: () => ({
        key: "trans",
        label: "Translation",
        text: verse.translation || verse.translation_en || "",
      }),
      wfw: () => {
  ...
  function renderPage(scrollToTop = false) {
    ...
    for (let i = start; i < end; i++) {
      ...
      const href = `verse.html?book=${encodeURIComponent(
        row.work_id
    ...
    if (scrollToTop) {
      const top = Math.max(
        0,
        (document.getElementById("results").offsetTop || 0) - 80
      );
  ...
  function pickSnippetsRow(row, f, q) {
    ...
    const active = scopesOrder.filter((k) => f.scopes[k]);
    const out = [];
    if (active.includes("deva") && (row.deva || "").trim())
      out.push({ key: "deva", label: "Devanāgarī", text: row.deva });
    if (active.includes("iast") && (row.iast || "").trim())
      out.push({ key: "iast", label: "IAST", text: row.iast });
    if (active.includes("trans") && (row.trans || "").trim())
      out.push({ key: "trans", label: "Translation", text: row.trans });
    if (active.includes("wfw")) {
      const line = ALL.wfwByVerse.get(row.verse_id) || "";
      if (line.trim())
        out.push({ key: "wfw", label: "Word-for-word", text: line });
    }
    return out;
  }
  ...
  function initBookFilter() {
    ...
    (async () => {
      try {
        ...
        rows.forEach((b) => {
          if (!state.byType.has(b.type)) state.byType.set(b.type, []);
          state.byType.get(b.type).push(b.id);
        });
        render();
        document.dispatchEvent(
          new CustomEvent("books:changed", {
    ...
    function render() {
      if (!state.filtered.length) {
        listEl.innerHTML =
          '<div class="muted" aria-live="polite">No books</div>';
        return;
      }
      ...
      state.filtered.forEach((b) => {
        ...
        function updateParentFor(typeCode) {
          const typeBox = document.querySelector(
            `#typeChecklist input[data-type='${CSS.escape(typeCode)}']`
          );
          if (!typeBox) return;
          const typeList = state.byType.get(typeCode) || [];
          const selectedCount = typeList.filter((id) =>
            state.selected.has(id)
          ).length;
          if (selectedCount === 0) {
            typeBox.checked = false;
            typeBox.indeterminate = false;
          } else if (selectedCount === typeList.length) {
    ...
    if (bookClear)
      bookClear.addEventListener("click", () => {
        ...
        if (typeList) {
          ...
          document.dispatchEvent(
            new CustomEvent("types:changed", { detail: { selected: [] } })
          );
        }
        render();
        document.dispatchEvent(
          new CustomEvent("books:changed", { detail: { selected: [] } })
        );
        listEl.dispatchEvent(new Event("change", { bubbles: true }));
      });
  }
    ...
    (async () => {
      ...
      types.forEach((t, idx) => {
        ...
        box.addEventListener("change", () => {
          ...
          const bookBoxes = document.querySelectorAll(
            `#fBooks input[type='checkbox']`
          );
          bookBoxes.forEach((bx) => {
            const row = ALL.books.find((x) => x.id === bx.value);
            if (row && row.type === t.code) {
              if (bx.checked !== box.checked) {
                bx.checked = box.checked;
